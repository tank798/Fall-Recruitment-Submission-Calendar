...
async function main() {
  ...
  try {
    ...
    const newYearInterview = legacy.schedules.find((item) => item.detail === "终面");
    const writtenExam = legacy.schedules.find((item) => item.detail === "笔试");
    check("省略年份的跨年进度推断正确", newYearInterview?.date === "2027-01-15");
    check("进度时间与阶段保留", writtenExam?.time === "09:00" && writtenExam.stage === "笔试");
    check(
      "无投递日期的进展没有丢失",
    ...
    check(
      ...
      stageStore.schedules.filter((item) => item.stage === "面试").length === 3 &&
        stageStore.schedules.some((item) => item.detail === "线上机考" && item.stage === "笔试"),
    );
    ...
    check(
      ...
      roundTripSchedule?.stage === "面试" &&
        roundTripSchedule.time === "15:00" &&
        roundTripSchedule.location === "腾讯会议" &&
        roundTripSchedule.sourceLink === "https://example.com/tencent" &&
        roundTripSchedule.detail === "一面",
    );
    check(
      "笔试轮次可以无损导出和回导",
      roundTrip.schedules.find((item) => item.id === "s2")?.writtenRound === "二",
    );
