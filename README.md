...
一个面向个人秋招管理的本地优先 Web 应用。它把分散在 Excel 里的投递、笔试（含测评）、面试、Offer 和未通过记录整理成清晰的时间表与横向时间轴，并允许直接在网页中新增、复制、修改和删除记录。
...
- 记录与浏览优先，只保留一排紧凑的岗位进展卡片，不做传统 Dashboard 或 KPI 大屏。
- 时间信息优先，减少装饰和无关图标。
...
项目把三种使用场景放在同一套轻量界面里：时间表负责逐条维护，时间轴负责连续浏览，公司与岗位负责查找岗位和 JD。五张进展卡片使用同一套“公司 + 岗位”去重口径，但不使用复杂的后台式菜单。

### 时间表：一行就是一条秋招记录

顶部五张卡片展示已投岗位总数以及笔试、面试、Offer、未通过的岗位数。数字按“公司 + 岗位”去重；第一张“投递”卡代表全部已投岗位，用于回到全量视图。下方表格将日期、公司、岗位、批次和环节横向对齐，点击任意一行即可打开详情抽屉。

![时间表：按日期倒序查看日程](docs/screenshots/01-timetable.png)

### 时间轴：连续浏览每天的安排

横向日期列覆盖当前届别的完整日期范围，可以左右滑动查看前后日期；投递、笔试（含测评）、面试、Offer 和未通过使用小型彩色标记区分，适合快速扫一眼接下来几天的安排。
...
- 按公司、岗位或环节进行模糊搜索。
- 点击笔试、面试、Offer 或未通过卡片筛选记录；再点一次取消筛选。点击“投递”卡回到全部已投岗位。
- 通过“添加”按钮创建新日程。
- 点击任意日程打开右侧详情抽屉。
- 在详情中查看、修改或删除日程。
...
网页表单面向当前工作流，字段包括公司、岗位、日期、招聘批次、环节、环节细分、来源链接和 JD。选择面试后，会在该选项正下方出现一个紧凑输入框，可以填写 `AI`、`一`、`二` 或 `终`，页面对应显示为“AI面”“一面”“二面”或“终面”；选择笔试后可以填写 `一`、`二` 或 `三`，对应显示为“一笔”“二笔”或“三笔”；选择 Offer 后填写“实习”或“正式”，右侧固定后缀会组合为“实习 Offer”或“正式 Offer”。
...
当前日程模型聚焦公司、岗位、日期、招聘批次、环节细分、来源链接和 JD。旧版单独的“测评”环节会自动迁移为“笔试 / 测评”，避免升级后丢失记录。

### 时间轴

时间轴是横向 Calendar Dashboard，与时间表复用同一份数据，因此新增、修改或删除记录后会同步更新。

时间轴的核心规则如下：

- 一列代表一个自然日，当前届别范围内的每一天都会生成日期列。
- 顶部月份刻度与日期列同步，可快速跳转到某月或回到今天。日期区域支持触控板横向滑动、Shift + 鼠标滚轮、触摸滑动和鼠标拖拽，并使用按日吸附对齐。
...
- 同一天先按“Offer > 面试 > 笔试 > 投递 > 未通过”排序，同一环节内再按公司名称排列。
- 顶部五张卡片与时间表使用同一套去重统计和筛选状态。
- 点击事件卡片继续复用时间表的详情抽屉。

卡片只展示浏览所需的公司头像、公司名和环节或具体轮次，不在时间轴里堆叠 JD 和长文本。

### 公司与岗位

该页面读取岗位数据，以紧凑的公司卡片展示已投递公司和岗位。桌面宽度下一行最多排列六张公司卡片，并提供公司或岗位搜索。

顶部进展卡片同时作为岗位筛选器：例如点击“面试”后，列表只保留存在面试记录的公司和岗位。搜索框在搜索期间保持展开，点击框内叉号后才会清空并收起。
...
这套规则由 `lib/cohort.ts` 统一计算，没有在各页面分别写死。届别选择器是全局状态；切换后，时间表、时间轴、公司与岗位页面以及新增日程的日期约束会立即同步。搜索、届别、导出 Excel 和添加操作收纳在全局顶部栏。
